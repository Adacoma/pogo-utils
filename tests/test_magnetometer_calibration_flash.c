...
void write_page_flash(uint8_t page, const void *data) {
    ...
    for (size_t i = 0; i < MAGNETOMETER_CALIBRATION_FLASH_PAGE_SIZE; ++i) {
        fake_flash[page][i] &= source[i];
    }
...
int main(void) {
    ...
    assert(magnetometer_calibration_flash_erase_store(&detector, &metadata) ==
    ...
    memcpy(saved_page, fake_flash[MAGNETOMETER_CALIBRATION_FLASH_PAGE], sizeof(saved_page));
    fake_flash[MAGNETOMETER_CALIBRATION_FLASH_PAGE][20] ^= 1u;
    unchanged = loaded;
    ...
    memcpy(fake_flash[MAGNETOMETER_CALIBRATION_FLASH_PAGE], saved_page, sizeof(saved_page));

    fake_flash[MAGNETOMETER_CALIBRATION_FLASH_PAGE][0] = 'X';
    assert(magnetometer_calibration_flash_load(&loaded, NULL) ==
           MAGNETOMETER_CALIBRATION_FLASH_BAD_MAGIC);
    memcpy(fake_flash[MAGNETOMETER_CALIBRATION_FLASH_PAGE], saved_page, sizeof(saved_page));

    fake_flash[MAGNETOMETER_CALIBRATION_FLASH_PAGE][4] = 2u;
    assert(magnetometer_calibration_flash_load(&loaded, NULL) ==
           MAGNETOMETER_CALIBRATION_FLASH_UNSUPPORTED_VERSION);
    memcpy(fake_flash[MAGNETOMETER_CALIBRATION_FLASH_PAGE], saved_page, sizeof(saved_page));

    fake_flash[MAGNETOMETER_CALIBRATION_FLASH_PAGE][6] = 0u;
    assert(magnetometer_calibration_flash_load(&loaded, NULL) ==
    ...
    assert(memcmp(fake_flash[MAGNETOMETER_CALIBRATION_FLASH_PAGE], saved_page,
                  sizeof(saved_page)) == 0);
    detector.model = valid_model();
    assert(magnetometer_heading_detection_set_offset(&detector, 0.25f));
    assert(magnetometer_heading_detection_set_filter_gain(&detector, 0.5f));
