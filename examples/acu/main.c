/**
 * @file main.c
 * @brief Static-parameter ACU controller with flash-calibrated heading.
 *
 ...
 * starts ACU control. Collection and fitting live only in calibration firmware.
 * The five ACU motility parameters are fixed after startup; this example has
 * no optimizer, fitness, genotype exchange, HIT, FT, or allocator state.
 *
 ...
 * escape heading. ACU is suspended through the FULL turn/settle/commit,
 * including the exit tick; no inline PID, calibration or motor mixing remains.
 * The ACU motility law follows libs/ACU-selfadapt, while the integration and
 * wire protocol follow this repository's hardened Vicsek example.
...
/* ACU is intrinsically continuous and uses collective U-turns. Keep both in
 * simulator and hardware builds; the static noise parameter may still be zero. */
#ifndef ACU_ENABLE_CONTINUOUS_MODE
#define ACU_ENABLE_CONTINUOUS_MODE 1
#endif
#ifndef ACU_ENABLE_CLUSTER_UTURNS
#define ACU_ENABLE_CLUSTER_UTURNS 1
#endif
...
/* ACU supplies a heading increment and forward-speed ratio. The reusable PID,
 * motor calibration, and motion coordinator remain the only motor authority. */
float pid_kp = 0.60f;
float pid_ki = 0.10f;
float pid_kd = 0.04f;
...
/* -------------------------- Static ACU model ---------------------------- */
/* These values are loaded once by Pogosim or remain compiled defaults on a
 * robot. They never mutate after user_init and are never exchanged. Defaults
 * reproduce the reference fixed "flock + wall following" genotype after its
 * normalized beta/sigma values are converted by the source's factor of ten. */
#ifndef ACU_MAX_NEIGHBORS
#define ACU_MAX_NEIGHBORS 20u
#endif
#if ACU_MAX_NEIGHBORS < 1 || ACU_MAX_NEIGHBORS > 255
#error "ACU_MAX_NEIGHBORS must fit a nonzero uint8_t"
#endif
#ifndef ENABLE_ACU_UART
#define ENABLE_ACU_UART 0
#endif
uint32_t max_age = 600u;             /* Neighbor age measured on OUR receive clock. */
uint32_t beacon_period_ms = 50u;     /* At most one heading beacon per 20 Hz tick. */
int acu_rx_messages_per_tick = 16;   /* Bounded RX work; includes wall packets. */
bool include_self_in_avg = true;
bool broadcast_angle_when_avoiding_walls = true;
bool include_avoiding_neighbors = true;
float acu_beta_rad_per_s = 9.0f;
float acu_noise_sigma_rad_sqrt_s = 0.0f;
float acu_base_speed_ratio = 0.80f;
float acu_u_turn_angle_rad = 0.40f * PI_F;
float acu_crowd_depth = 0.0f;
float acu_target_neighbors = 6.0f;
float acu_crowd_width_neighbors = 4.0f;
float acu_crowd_factor_max = 2.0f;
float acu_max_dt_s = 0.05f;
uint32_t acu_log_period_ms = 1000u;

/* ACU collective U-turns begin on the rising edge of local wall avoidance.
 * The target is measured heading + the fixed phase above. Relative remaining
 * lifetime and a hop bound avoid comparing clocks or relaying forever. Local
 * wall avoidance retains priority while it owns the motion coordinator. */
#if ACU_ENABLE_CLUSTER_UTURNS
bool enable_cluster_u_turn = true;
 ...
 * Base (8 bytes): 'A','C',version,flags,id_le16,angle_mrad_le16.
 ...
 */
#define ACU_PROTOCOL_VERSION 1u
#define ACU_MSG_BYTES 8u
#define ACU_HINT_MSG_BYTES 16u
#define AMSGF_CLUSTER_UTURN 0x01u
#define AMSGF_AVOIDING 0x02u
#define AMSGF_MEASURED 0x04u
#define AMSG_HOPS_SHIFT 4u
...
typedef struct {
    ...
    neighbor_t neighbors[ACU_MAX_NEIGHBORS];
    uint8_t nb_neighbors;
    uint8_t nb_neighbors_used;
    float neighbor_order;
    float theta_mean_rad;
    float theta_acu_rad;          /* Last ACU proposal, may be ignored by WA. */
    float theta_cmd_rad;          /* Actual coordinator/PID target. */
    float acu_heading_increment_rad;
    float acu_effective_speed_ratio;
    uint32_t last_acu_update_ms;
    uint32_t last_acu_log_ms;
    uint32_t last_beacon_ms;
    uint32_t random_state;
    bool acu_paused;
    bool acu_suggestion_valid;
    bool previous_wall_owned;
    uint32_t acu_update_count;
    uint32_t acu_applied_count;
...
static void recover_runtime_avoidance_fault(uint32_t now) {
    ...
    mydata->acu_paused = true;
    mydata->acu_suggestion_valid = false;
    mydata->previous_wall_owned = false;
    mydata->cluster_turn_active = false;
    mydata->last_acu_update_ms = now;
    mydata->runtime_recovery_active = true;
}

static uint32_t acu_random_u32(void) {
...
static float uniform_open01(void) {
    ...
    return ((float)(acu_random_u32() >> 9) + 0.5f) * (1.0f / 8388608.0f);
}

#if ACU_ENABLE_CONTINUOUS_MODE
...
static neighbor_t *upsert_neighbor(uint16_t id, uint32_t now) {
    ...
    if (mydata->nb_neighbors >= ACU_MAX_NEIGHBORS) {
...
static void cluster_start_local(uint32_t now, float heading_rad) {
    if (!enable_cluster_u_turn || !isfinite(heading_rad)) {
        return;
    }
    /* Match the reference ACU trigger: a local wall encounter creates a fixed
     * phase-offset heading. Wall avoidance still owns this robot until its
     * full turn/settle/commit finishes; neighbors may react immediately. */
    mydata->cluster_target_rad = heading_wrap_pi(
        heading_rad + acu_u_turn_angle_rad);
...
bool send_message(void) {
    if (mydata->controller_state != CONTROLLER_ACU ||
    ...
    /* ACU aligns physical orientations, so packets always carry the measured
     * heading rather than the local PID/social target. */
    const bool measured = true;
    float advertised = heading.angle_rad;
    if (!isfinite(advertised)) {
        return false;
    }
#if ACU_ENABLE_CLUSTER_UTURNS
    uint8_t packet[ACU_HINT_MSG_BYTES] = {0};
#else
    uint8_t packet[ACU_MSG_BYTES] = {0};
#endif
    packet[0] = (uint8_t)'A';
    packet[1] = (uint8_t)'C';
    packet[2] = ACU_PROTOCOL_VERSION;
    packet[3] = (uint8_t)((avoiding ? AMSGF_AVOIDING : 0u) | (measured ? AMSGF_MEASURED : 0u));
    put_u16(packet + 4, pogobot_helper_getid());
    put_u16(packet + 6, (uint16_t)rad_to_mrad(advertised));
    uint16_t length = ACU_MSG_BYTES;
#if ACU_ENABLE_CLUSTER_UTURNS
    if (advertise_cluster) {
        packet[3] |= (uint8_t)(AMSGF_CLUSTER_UTURN | (mydata->cluster_hops_left << AMSG_HOPS_SHIFT));
...
void process_message(message_t *message) {
    if (message == NULL || mydata->controller_state != CONTROLLER_ACU) {
        return;
    }
...
#if ACU_ENABLE_CLUSTER_UTURNS
    bool valid_length = length == ACU_MSG_BYTES || length == ACU_HINT_MSG_BYTES;
#else
    bool valid_length = length == ACU_MSG_BYTES;
#endif
    if (!valid_length || p[0] != (uint8_t)'A' || p[1] != (uint8_t)'C' ||
        p[2] != ACU_PROTOCOL_VERSION) {
        counter_increment(&mydata->rx_ignored);
        return;
    }
    uint8_t flags = p[3];
#if ACU_ENABLE_CLUSTER_UTURNS
    bool has_cluster = (flags & AMSGF_CLUSTER_UTURN) != 0u;
    uint8_t hops = (uint8_t)(flags >> AMSG_HOPS_SHIFT);
#else
    ...
    bool invalid_protocol = (flags & 0x08u) != 0u || !encoded_angle_valid(theta) ||
        (has_cluster ? (length != ACU_HINT_MSG_BYTES || hops > CLUSTER_MAX_HOPS ||
            !encoded_angle_valid(get_i16(p + 12))) :
            (length != ACU_MSG_BYTES || hops != 0u));
#else
    bool invalid_protocol = (flags & (uint8_t)~(AMSGF_AVOIDING | AMSGF_MEASURED)) != 0u ||
    ...
    if (neighbor != NULL) {
        ...
        neighbor->avoiding = (flags & AMSGF_AVOIDING) != 0u;
...
/* ----------------------- ACU target, never motor control ------------------ */

static float acu_effective_speed(void) {
    float deviation = fabsf((float)mydata->nb_neighbors - acu_target_neighbors);
    float normalized = deviation / acu_crowd_width_neighbors;
    if (normalized > 1.0f) {
        normalized = 1.0f;
    }
    /* V-shaped crowding law from ACU-selfadapt: motion is slowest at the
     * target degree and returns to base speed outside the configured width. */
    float depth = acu_crowd_factor_max * acu_crowd_depth;
    float factor = 1.0f - depth * (1.0f - normalized);
    factor = heading_clamp(factor, 0.0f, 1.0f);
    return heading_clamp(acu_base_speed_ratio * factor, 0.0f, 1.0f);
}

static float acu_target_increment(
    const heading_sample_t *heading, uint32_t now, float *forward_ratio) {
    *forward_ratio = acu_effective_speed();
    mydata->acu_effective_speed_ratio = *forward_ratio;
    ...
    mydata->acu_paused = !available;
    mydata->acu_suggestion_valid = false;
    mydata->acu_heading_increment_rad = 0.0f;
    if (!available) {
        mydata->last_acu_update_ms = now;
        return 0.0f;
    }
    uint32_t elapsed = (uint32_t)(now - mydata->last_acu_update_ms);
    if (elapsed == 0u) {
        return 0.0f;
    }
    mydata->last_acu_update_ms = now;
    ...
    float desired = have_mean ? mean : heading->angle_rad;
#if ACU_ENABLE_CLUSTER_UTURNS
    if (cluster_window_active(now)) {
        desired = mydata->cluster_target_rad;
    }
#endif
    ...
    float delta = acu_beta_rad_per_s *
        sinf(heading_wrap_pi(desired - heading->angle_rad)) * dt;
#if ACU_ENABLE_CONTINUOUS_MODE
    if (acu_noise_sigma_rad_sqrt_s > 0.0f) {
        delta += acu_noise_sigma_rad_sqrt_s * sqrtf(dt) * noise_gaussian();
    }
#endif
    float target = heading_wrap_pi(heading->angle_rad + delta);
    mydata->acu_heading_increment_rad = delta;
    mydata->theta_acu_rad = target;
    mydata->acu_suggestion_valid = true;
    counter_increment(&mydata->acu_update_count);
...
static void acu_after_motion(uint32_t now, float heading_rad) {
    bool wall_owned = wall_owns_heading();
#if ACU_ENABLE_CLUSTER_UTURNS
    if (wall_owned && !mydata->previous_wall_owned) {
        /* A local wall event supersedes an incoming event and starts a fresh
         * ACU U-turn. It is first advertised after this coordinator step. */
        mydata->cluster_turn_active = false;
        cluster_start_local(now, heading_rad);
    }
#else
    (void)now;
#endif
    mydata->previous_wall_owned = wall_owned;
    if (mydata->acu_suggestion_valid && mydata->drive.behavior == DDK_BEHAVIOR_NORMAL) {
        counter_increment(&mydata->acu_applied_count);
    }
...
static void acu_log_step(uint32_t now) {
    if (!ENABLE_ACU_UART ||
        (uint32_t)(now - mydata->last_acu_log_ms) < acu_log_period_ms) {
        return;
    }
    mydata->last_acu_log_ms = now;
    printf("ACU,id=%u,ms=%lu,neighbors=%u,used=%u,paused=%u,order_permille=%d,"
           "heading_mrad=%d,target_mrad=%d,speed_permille=%d,updates=%lu,applied=%lu,"
           "rx=%lu,tx=%lu,cluster=%u\n",
           (unsigned)pogobot_helper_getid(), (unsigned long)now,
           (unsigned)mydata->nb_neighbors, (unsigned)mydata->nb_neighbors_used,
           (unsigned)mydata->acu_paused, round_float_to_int(mydata->neighbor_order * 1000.0f),
           (int)rad_to_mrad(mydata->heading_detection.heading_rad),
           (int)rad_to_mrad(mydata->theta_cmd_rad),
           round_float_to_int(mydata->acu_effective_speed_ratio * 1000.0f),
           (unsigned long)mydata->acu_update_count, (unsigned long)mydata->acu_applied_count,
...
static void acu_enter(void) {
    diff_drive_kin_reset(&mydata->drive);
    mydata->controller_state = CONTROLLER_ACU;
    uint32_t now = now_ms();
    mydata->last_pid_log_ms = now;
    mydata->last_acu_update_ms = now;
    mydata->last_acu_log_ms = now;
    mydata->last_beacon_ms = now - acu_random_u32() % beacon_period_ms;
    mydata->nb_neighbors = 0u;
    mydata->cluster_turn_active = false;
    mydata->previous_wall_owned = false;
    mydata->acu_paused = true;
    mydata->acu_heading_increment_rad = 0.0f;
    mydata->acu_effective_speed_ratio = acu_base_speed_ratio;
...
static bool controller_configuration_is_valid(void) {
    return max_age > 0u && max_age < POGO_HEADING_HALF_TIME_RANGE &&
        ...
        acu_rx_messages_per_tick > 0 && acu_rx_messages_per_tick <= 100 &&
        isfinite(acu_beta_rad_per_s) && acu_beta_rad_per_s >= 0.0f &&
        acu_beta_rad_per_s <= 1000.0f &&
        isfinite(acu_noise_sigma_rad_sqrt_s) && acu_noise_sigma_rad_sqrt_s >= 0.0f &&
        acu_noise_sigma_rad_sqrt_s <= 100.0f &&
        isfinite(acu_base_speed_ratio) && acu_base_speed_ratio > 0.0f &&
        acu_base_speed_ratio <= 1.0f &&
        isfinite(acu_u_turn_angle_rad) && fabsf(acu_u_turn_angle_rad) <= 2.0f * PI_F &&
        isfinite(acu_crowd_depth) && acu_crowd_depth >= 0.0f &&
        acu_crowd_depth <= 1.0f &&
        isfinite(acu_target_neighbors) && acu_target_neighbors >= 0.0f &&
        acu_target_neighbors <= (float)ACU_MAX_NEIGHBORS &&
        /* The reference ACU clamps crowd width to at least one neighbor. */
        isfinite(acu_crowd_width_neighbors) && acu_crowd_width_neighbors >= 1.0f &&
        acu_crowd_width_neighbors <= (float)ACU_MAX_NEIGHBORS &&
        isfinite(acu_crowd_factor_max) && acu_crowd_factor_max >= 0.0f &&
        acu_crowd_factor_max <= 10.0f &&
        isfinite(acu_max_dt_s) && acu_max_dt_s > 0.0f && acu_max_dt_s <= 1.0f &&
        acu_log_period_ms > 0u && acu_log_period_ms < POGO_HEADING_HALF_TIME_RANGE &&
...
void user_init(void) {
    ...
    main_loop_hz = 20;
    max_nb_processed_msg_per_tick = acu_rx_messages_per_tick;
    /* Give the callback every tick; beacon_period_ms is the sole TX rate gate.
     * Do not reduce TX twice using both callback probability and the timer. */
    ...
    if (!controller_configuration_is_valid()) {
        enter_fatal_state("invalid ACU/PID configuration");
        return;
    }
...
#if ENABLE_STARTUP_UART
    /* Integer-scaled values keep the hardware log independent of float printf. */
    printf("# ACU_CONFIG,robot=%u,wa_api=%u,protocol=%u,loop_hz=%d,beacon_ms=%lu,"
           "cluster=%u,max_neighbors=%u,beta_milli=%d,sigma_milli=%d,"
           "speed_permille=%d,uturn_mrad=%d,crowd_depth_permille=%d\n",
           (unsigned)pogobot_helper_getid(), (unsigned)WALL_AVOIDANCE_MAGNETOMETER_API_VERSION,
           (unsigned)ACU_PROTOCOL_VERSION, main_loop_hz, (unsigned long)beacon_period_ms,
           (unsigned)enable_cluster_u_turn, (unsigned)ACU_MAX_NEIGHBORS,
           round_float_to_int(acu_beta_rad_per_s * 1000.0f),
           round_float_to_int(acu_noise_sigma_rad_sqrt_s * 1000.0f),
           round_float_to_int(acu_base_speed_ratio * 1000.0f),
           round_float_to_int(acu_u_turn_angle_rad * 1000.0f),
           round_float_to_int(acu_crowd_depth * 1000.0f));
#endif
...
void user_step(void) {
    ...
    heading_sample_t input = heading_snapshot(now);
    purge_old_neighbors(now);
    float forward_ratio = acu_base_speed_ratio;
    float dtheta = acu_target_increment(&input, now, &forward_ratio);
    (void)diff_drive_kin_step_with_heading(&mydata->drive,
        forward_ratio, dtheta, &input, now);
    acu_after_motion(now, input.angle_rad);
...
static void create_data_schema(void) {
    ...
    data_add_column_double("theta_acu_rad");
    data_add_column_double("acu_heading_increment_rad");
    data_add_column_double("acu_effective_speed_ratio");
    data_add_column_double("acu_beta_rad_per_s");
    data_add_column_double("acu_noise_sigma_rad_sqrt_s");
    data_add_column_double("acu_base_speed_ratio");
    data_add_column_double("acu_u_turn_angle_rad");
    data_add_column_double("acu_crowd_depth");
    data_add_column_double("acu_target_neighbors");
    data_add_column_double("acu_crowd_width_neighbors");
    data_add_column_double("acu_crowd_factor_max");
    data_add_column_int8("acu_paused");
    data_add_column_int8("acu_suggestion_valid");
    data_add_column_int32("acu_updates");
    data_add_column_int32("acu_applied");
    data_add_column_int32("acu_rx");
    data_add_column_int32("acu_tx_attempts");
    data_add_column_int32("acu_capacity_drops");
...
static void export_data(void) {
    ...
    data_set_value_double("theta_acu_rad", (double)mydata->theta_acu_rad);
    data_set_value_double("acu_heading_increment_rad",
                          (double)mydata->acu_heading_increment_rad);
    data_set_value_double("acu_effective_speed_ratio",
                          (double)mydata->acu_effective_speed_ratio);
    data_set_value_double("acu_beta_rad_per_s", (double)acu_beta_rad_per_s);
    data_set_value_double("acu_noise_sigma_rad_sqrt_s",
                          (double)acu_noise_sigma_rad_sqrt_s);
    data_set_value_double("acu_base_speed_ratio", (double)acu_base_speed_ratio);
    data_set_value_double("acu_u_turn_angle_rad", (double)acu_u_turn_angle_rad);
    data_set_value_double("acu_crowd_depth", (double)acu_crowd_depth);
    data_set_value_double("acu_target_neighbors", (double)acu_target_neighbors);
    data_set_value_double("acu_crowd_width_neighbors",
                          (double)acu_crowd_width_neighbors);
    data_set_value_double("acu_crowd_factor_max", (double)acu_crowd_factor_max);
    data_set_value_int8("acu_paused", (int8_t)mydata->acu_paused);
    data_set_value_int8("acu_suggestion_valid", (int8_t)mydata->acu_suggestion_valid);
    data_set_value_int32("acu_updates", (int32_t)(mydata->acu_update_count > INT32_MAX ? INT32_MAX : mydata->acu_update_count));
    data_set_value_int32("acu_applied", (int32_t)(mydata->acu_applied_count > INT32_MAX ? INT32_MAX : mydata->acu_applied_count));
    data_set_value_int32("acu_rx", (int32_t)(mydata->rx_headings > INT32_MAX ? INT32_MAX : mydata->rx_headings));
    data_set_value_int32("acu_tx_attempts", (int32_t)(mydata->tx_attempts > INT32_MAX ? INT32_MAX : mydata->tx_attempts));
    data_set_value_int32("acu_capacity_drops", (int32_t)(mydata->neighbor_capacity_drops > INT32_MAX ? INT32_MAX : mydata->neighbor_capacity_drops));
...
static void global_setup(void) {
    ...
    init_from_configuration(acu_rx_messages_per_tick);
    ...
    init_from_configuration(acu_beta_rad_per_s);
    init_from_configuration(acu_noise_sigma_rad_sqrt_s);
    init_from_configuration(acu_base_speed_ratio);
    init_from_configuration(acu_u_turn_angle_rad);
    init_from_configuration(acu_crowd_depth);
    init_from_configuration(acu_target_neighbors);
    init_from_configuration(acu_crowd_width_neighbors);
    init_from_configuration(acu_crowd_factor_max);
    init_from_configuration(acu_max_dt_s);
    init_from_configuration(acu_log_period_ms);
