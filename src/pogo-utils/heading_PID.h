 ...
 *   6) float u = heading_pid_update(&pid);     // call every control tick
...
typedef struct {
    ...
    float u_max;                   ///< Absolute output saturation (|u| <= u_max, typically <= 1.0)
    float I_max;                   ///< Absolute integrator clamp (|I| <= I_max)

    // --- target/state ---
    float target_rad;              ///< Target heading (radians), wrapped to (-pi, pi]
    float e_prev;                  ///< Previous error for derivative term
    float I;                       ///< Integrator state
    float u;                       ///< Last computed control (post-saturation)
